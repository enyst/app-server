...
Ported from ``tests/unit/storage/data_models/test_llm_profiles.py`` in
OpenHands/OpenHands (at ee9e78b7). The managed-proxy base_url resolution case is
dropped along with the cloud LiteLLM proxy it resolved against.

Settings-integration tests live in test_settings_models.py; the HTTP surface is
covered in test_llm_profiles.py.
"""
...
from app_server.llm_profiles import (
    MAX_PROFILES,
    LLMProfiles,
...
def _make_llm(model: str = "openai/gpt-4o", api_key: str | None = None) -> LLM:
    return LLM(model=model, api_key=SecretStr(api_key) if api_key is not None else None)
...
def test_summaries_returns_name_model_base_url_and_api_key_set():
    ...
    profiles.save("p2", LLM(model="anthropic/claude-opus-4", base_url="https://example.com"))
...
def test_active_stays_in_profiles_at_all_entry_points():
    """``active`` must point at an existing profile — enforced both at validate
    time (loading corrupted state) and at assignment time."""
    # Validate-time: an orphan active in persisted data is auto-cleared.
    loaded = LLMProfiles.model_validate({"profiles": {"a": {"model": "openai/gpt-4o"}}, "active": "ghost"})
    assert loaded.active is None

    # Assignment-time: setting an unknown name clears; a known one sticks.
    profiles = LLMProfiles()
    profiles.save("a", _make_llm())
...
def test_orphan_active_heals_on_roundtrip():
    """Disaster recovery: if something bypasses the invariant (a manual file
    edit, old data), the next validate cycle drops the dangling pointer."""
    profiles = LLMProfiles()
    profiles.save("real", _make_llm())
    object.__setattr__(profiles, "active", "ghost")  # bypass validator

    rehydrated = LLMProfiles.model_validate(profiles.model_dump(mode="json"))
...
def test_save_fails_past_limit():
    ...
    assert exc_info.value.limit == MAX_PROFILES
...
def test_strict_llm_rejects_unknown_fields():
    with pytest.raises(ValidationError):
        StrictLLM.model_validate({"model": "openai/gpt-4o", "totally_made_up_field": "x"})
